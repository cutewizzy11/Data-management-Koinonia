...
const DetailView = ({ person, type, onClose, onPersonClick, onNavigateToTab }: DetailViewProps) => {
  ...
  const fetchRealTimeData = async () => {
    ...
    try {
      ...
      if (import.meta.env.DEV) console.error('[DetailView] Error:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (type === 'applicant') fetchRealTimeData();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [person.name, type]);

  // Use real-time data if available, else fallback to props
      ...
      <div className="bg-card rounded-xl shadow-strong max-w-6xl w-full max-h-[95vh] overflow-hidden animate-scale-in">
        ...
        <div className="bg-gradient-subtle border-b border-border p-6">
          <div className="flex items-center justify-between mb-6">
            <button onClick={onClose} className="nav-item flex items-center space-x-2">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button onClick={onClose} className="p-2 hover:bg-secondary/50 rounded-md transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
          </div>
          ...
          <div className="flex items-start space-x-6">
            ...
            <div className={`w-32 h-32 rounded-2xl ${getAvatarBg()} overflow-hidden flex-shrink-0 shadow-lg`}>
              ...
              <div className={`w-full h-full flex items-center justify-center text-white font-bold text-2xl ${imageUrl && !isResolving ? 'hidden' : ''}`}>
            ...
            <div className="flex-1">
              ...
              <p className="text-xl text-muted-foreground mb-6">{getRoleLabel()}</p>
        ...
        <div className="p-6 overflow-y-auto max-h-[60vh]">
          ...
          {error && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
              <div className="flex items-center space-x-2">
                ...
                <button onClick={fetchRealTimeData} className="ml-auto text-red-600 hover:text-red-800 underline">Retry</button>
              </div>
            </div>
          )}

          {/* Applicant: Referees & Associates */}
          {type === 'applicant' && (
            <div className="space-y-8">
              ...
              <div>
                <div className="flex items-center space-x-3 mb-6">
                  ...
                  {realTimeData && (
                    <span className="text-xs bg-orange-100 text-orange-800 px-3 py-1 rounded-full font-medium">Live Data</span>
                  )}
                </div>

                {displayData.associates && displayData.associates.length > 0 ? (
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    {displayData.associates.map((associate) => (
                      <PersonCard
                        key={associate.id}
                        person={associate}
                        type="associate"
                        onClick={() => {
                          if (onNavigateToTab) onNavigateToTab('associates', associate);
                          else onPersonClick(associate, 'associate');
                        }}
                        showRelations={false}
                      />
                    ))}
                  ...
                  <div className="text-center py-8 bg-secondary/20 rounded-lg">
                    ...
                    <p className="text-muted-foreground">{loading ? 'Loading associates...' : 'No associates found'}</p>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                {displayData.referees && displayData.referees.length > 0 ? (
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    {displayData.referees.map((referee) => (
                      <PersonCard
                        key={referee.id}
                        person={referee}
                        type="referee"
                        onClick={() => {
                          if (onNavigateToTab) onNavigateToTab('referees', referee);
                          else onPersonClick(referee, 'referee');
                        }}
                        showRelations={false}
                      />
                    ))}
                  ...
                  <div className="text-center py-8 bg-secondary/20 rounded-lg">
                    ...
                    <p className="text-muted-foreground">{loading ? 'Loading referees...' : 'No referees found'}</p>
                  </div>
                )}
              </div>
          ...
          {/* Referee/Associate: show related applicant */}
          {(type === 'referee' || type === 'associate') && person.applicant && (
            <div>
              <div className="flex items-center space-x-3 mb-6">
                <Users className="w-6 h-6 text-accent" />
                <h3 className="text-2xl font-bold text-foreground">Related Applicant</h3>
              </div>
          ...
          <div className="px-6 pb-6">
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-4 rounded-lg border md:col-span-2">
                ...
                <div className="font-medium">{(person as any).reasonLeftFormerEmployer || (person as any).reasonForLeavingFormerEmployer || '—'}</div>
              </div>
            </div>
          </div>
        )}
